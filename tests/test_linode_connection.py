...
class LinodeConnectionTestCase(CloudTestMixin, TestCase):
    def setUp(self):
        super().setUp()
    ...
    def test_valid_token_connection(self, mock_get):
        ...
        mock_get.return_value = self.create_mock_response('linode', 'account_success')
        ...
        self.assert_api_called_with_credentials(mock_get, 'linode', account_config)

    @patch('requests.get')
    def test_invalid_token_connection(self, mock_get):
        # Get invalid test account configuration
        account_config = self.get_test_account('linode', 'invalid')
        mock_get.return_value = self.create_mock_response('linode', 'account_unauthorized')
    ...
    def test_access_token_property(self):
        ...
        self.assertEqual(linode_account.access_token, account_config['access_token'])

    @skip_if_no_real_credentials('linode')
    def test_real_api_connection(self):
        """
        Integration test with real Linode API (only runs if real credentials provided)
        """
        account_config = self.get_test_account('linode', 'valid')
        ...
        result = linode_account.validate()
        self.assertTrue(result, "Real Linode API validation should succeed with valid credentials")
    ...
    def test_account_suspended_connection(self, mock_get):
        # Get suspended test account configuration
        account_config = self.get_test_account('linode', 'suspended')
