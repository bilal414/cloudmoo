#!/usr/bin/env bash
# CloudMoo server installer.
#
# Run from a trusted terminal with:
#   curl -fsSL https://raw.githubusercontent.com/bilal414/cloudmoo/main/install.sh | sudo bash
#
# This installer deliberately supports the two distributions that the CloudMoo
# Docker image and deployment documentation are tested on: Ubuntu and Debian.

set -Eeuo pipefail
IFS=$'\n\t'

readonly REPOSITORY_URL="https://github.com/bilal414/cloudmoo.git"
readonly DEFAULT_BRANCH="main"
readonly DEFAULT_INSTALL_DIR="/opt/cloudmoo"
readonly APP_PORT="8000"

BRANCH="$DEFAULT_BRANCH"
INSTALL_DIR="$DEFAULT_INSTALL_DIR"
PUBLIC_HOST="${CLOUDMOO_DOMAIN:-}"
SKIP_START=false
ENV_FILE=""
APP_DOMAIN=""
...
usage() {
    cat <<'EOF'
Install CloudMoo on a Debian or Ubuntu server.
...
Options:
  ...
  --install-dir PATH  Installation directory (default: /opt/cloudmoo).
...
set_env_value() {
    ...
    # All values passed here are generated hex strings or validated hostnames/URLs, so
    # they cannot contain a quote or the sed delimiter.
...
clone_or_reuse_repository() {
    if [[ -e "$INSTALL_DIR" ]]; then
        if [[ -f "$INSTALL_DIR/docker-compose.yml" && -f "$INSTALL_DIR/.env.example" ]]; then
            log "Using existing CloudMoo installation at ${INSTALL_DIR}"
            return
        fi

        if [[ ! -d "$INSTALL_DIR" || -n "$(ls -A -- "$INSTALL_DIR")" ]]; then
            die "${INSTALL_DIR} already exists and is not a CloudMoo installation. Choose an empty --install-dir."
        fi
    fi

    log "Downloading CloudMoo (${BRANCH})"
...
create_or_keep_env_file() {
    ...
    set_env_value DJANGO_SERVER "production"
    set_env_value DB_PASSWORD "$(random_hex 24)"
    set_env_value DB_HOST "db"
    # Celery broker: the internal RabbitMQ service from docker-compose.yml.
    set_env_value CELERY_BROKER_URL "amqp://guest:guest@rabbitmq:5672//"
    set_env_value DJANGO_ALLOWED_HOSTS "${PUBLIC_HOST},localhost,127.0.0.1"
    set_env_value DJANGO_CSRF_TRUSTED_ORIGINS "http://${APP_DOMAIN}"
    set_env_value APP_DOMAIN "$APP_DOMAIN"
    set_env_value APP_PROTOCOL "http://"
    set_env_value HTTPS_ENABLED "false"
    chmod 600 "$ENV_FILE"
}
...
warn_on_low_resources() {
    ...
    if [[ -r /proc/meminfo ]]; then
        ...
        if [[ -n "$available_kb" && "$available_kb" -lt 1572864 ]]; then
            warn "Less than 1.5 GiB of RAM is currently available. The stack may need more memory under load."
        fi
    fi
}
...
start_stack() {
    ...
    log "Building and starting CloudMoo (the first build can take several minutes)"
    (
        cd "$INSTALL_DIR"
...
show_logs() {
    (
        cd "$INSTALL_DIR"
        docker compose logs --tail 100 web migrate || true
    )
}
...
wait_for_app() {
    ...
    log "Waiting for CloudMoo to become ready"
    while [[ "$elapsed" -lt 300 ]]; do
        ...
        container_id="$(cd "$INSTALL_DIR" && docker compose ps --all -q web 2>/dev/null || true)"
        if [[ -n "$container_id" ]]; then
            status="$(docker inspect --format '{{.State.Status}}' "$container_id" 2>/dev/null || true)"
            case "$status" in
                exited|dead)
                    show_logs
                    die "CloudMoo did not start successfully (web state: ${status})."
                    ;;
            esac
        fi

        if curl -fsS --max-time 5 "http://localhost:${APP_PORT}/healthz/" >/dev/null 2>&1; then
            return
        fi
    ...
    show_logs
    die "CloudMoo did not become ready within five minutes."
}
...
print_next_steps() {
    ...
    printf '\n'
    printf 'CloudMoo is running.\n\n'
    printf 'Open: %s\n' "$app_url"
    printf 'Create the first admin user with:\n'
    printf '  cd %s && docker compose exec web python manage.py createsuperuser\n' "$INSTALL_DIR"
    ...
    printf 'Before exposing this instance publicly, put a TLS reverse proxy in front of it and set\n'
    printf 'HTTPS_ENABLED=true, APP_PROTOCOL=https://, and DJANGO_CSRF_TRUSTED_ORIGINS in %s/.env.\n' "$INSTALL_DIR"
}
...
main() {
    ...
    if [[ "$SKIP_START" == true ]]; then
        ...
        printf 'Start CloudMoo with: cd %s && docker compose up --build -d\n' "$INSTALL_DIR"
        return
    fi
